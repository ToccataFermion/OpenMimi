...
**Wave 2 #12 (待开始)**:结构化 ToolResult — 给 `ToolResult` 加可选 `structured: dict[str, Any]` 字段,让那些已经在内部构造 dict / list 的 handler(如 `extract`、`get_box`、`is_visible`、`network_log`、`storage` 等)能把原始结构化数据通过 `structured` 字段传出,与现有的 `output: str`(LLM 可读文本)并存,便于上层(orchestrator / planner)做精确判断而不需要再 re-parse 文本。先在 `tools/result.py` 加字段,再挑 2-3 个高价值 handler 改写示范,加测试。`#4-lite` 已完结(主类 773 行,远低于 < 1000 行目标)。
