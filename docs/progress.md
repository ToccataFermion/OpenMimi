...
**Wave 2 #4-lite (进行中)**:`agent_browser.py` dispatcher 化。前 6 步已完成 — `actions/` 包脚手架 + 4 nav + 11 interaction + 4 scroll + 11 extract + 5 wait + 8 tab/session action 外迁(共 43 个)。`agent_browser.py` 当前 2257 行(从 3766 起步,目标 < 1000)。下一步:迁 network/cdp/storage/pdf/console/screenshot 族。
