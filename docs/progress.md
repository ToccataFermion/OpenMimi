...
**Wave 2 #4-lite (进行中)**:`agent_browser.py` dispatcher 化。前 3 步已完成 — `actions/` 包脚手架 + 4 个导航 action + 11 个交互 action + 4 个 scroll action 外迁。`agent_browser.py` 当前 3165 行(从 3766 起步,目标 < 1000)。下一步:迁 extract / page-state 族(snapshot/extract/page_source/get_url/get_title/get_attribute/set_attribute/get_property/get_box/is_visible/visual_locate)。
