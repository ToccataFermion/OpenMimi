...
**Wave 2 #4-lite (进行中)**:`agent_browser.py` dispatcher 化。前 5 步已完成 — `actions/` 包脚手架 + 4 nav + 11 interaction + 4 scroll + 11 extract + 5 wait action 外迁(共 35 个)。`agent_browser.py` 当前 2515 行(从 3766 起步,目标 < 1000)。下一步:迁 tab/session 族(tab_list/tab_switch/tab_new/tab_close/save_session/load_session/clipboard/clear_cache)。
...
- [ ] #4-lite `agent_browser.py` dispatcher 化(拆 `actions/` 子目录,主类只做路由 + daemon 管理)
  ...
  - [ ] step 6 — tab/session 族(tab_*/save_session/load_session/clipboard/clear_cache)
  - [ ] step 7 — network/cdp/storage/pdf/console/screenshot 族
