...
**Wave 2 #4-lite (进行中)**:`agent_browser.py` dispatcher 化。前 4 步已完成 — `actions/` 包脚手架 + 4 个导航 + 11 个交互 + 4 个 scroll + 11 个 extract/page-state action 外迁(共 30 个)。`agent_browser.py` 当前 2734 行(从 3766 起步,目标 < 1000)。下一步:迁 wait 族(wait/wait_for/wait_for_disappear/wait_for_navigation/wait_for_network_idle)。
