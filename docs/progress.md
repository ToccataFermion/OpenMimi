...
**Wave 2 #4-lite (进行中)**:`agent_browser.py` dispatcher 化。前 8 步已完成 — `actions/` 包脚手架 + 4 nav + 11 interaction + 4 scroll + 11 extract + 5 wait + 8 tab/session + 7 network/cdp + 13 misc action 外迁(共 63 个)。`agent_browser.py` 当前 1174 行(从 3766 起步,目标 < 1000)。下一步:step 9 主类瘦身 — 主类只剩 `__init__` / `__call__` / `_dispatch` / daemon 管理。
